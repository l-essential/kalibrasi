...
class Meet_progress extends MY_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        $this->load->model($this->pathclass . '/Meet_progress_model', 'modeldata');
        $this->setmodel($this->pathclass . '/Meet_progress_model#modeldataprogress');
    ...
    //     $this->data['url_post'] = site_url($this->controller . '/postprogress');
    //     $this->data['url_index'] = site_url($this->controller . "/load_detail");
    //     $this->load->view($this->view . '/form_progress', $this->data);
    // }

     public function load_detail($id) {
        $this->iddata = $id;
        $rowheader = $this->modeldata->getby_id_array($id);
        $this->data['prefix_id'] =$this->modeldata->prefix_idprogress;
        $this->data['id'] = 0;
        $this->data['progress_id'] = $id;
        $this->data['prefix_id'] = $this->prefix_id;
        $this->data["flagcondition"] = 'edit';
        ...
        //$rowrca = $this->modeldataprogress->getby_id_progress($id);
        $this->data['progressdetail'] =  $this->modeldataprogress->getby_id_progress($id);

       $this->data['controller'] = site_url($this->controller);
        $this->data['url_form_reply'] = site_url($this->controller . '/form_reply');
        $this->data['url_post'] = site_url($this->controller . '/postprogress');
        $this->data['url_index'] = site_url($this->controller . "/load_detail/$id");
        $this->load->view($this->view . '/form_progress', $this->data);
    }
     ...
     public function postprogress() {
        ...
        switch ($actiondata) {
            case "create":
                if (empty($checkdata)) {
                    $record = array(
                        "progressdetail_createby" => $param ['progressdetail_createby'],
                        "progress_id" => $param['progress_id'],
                        "progressdetail_text" => $param['progressdetail_text']
                    );
                    $last_id = $this->modeldata->insertprogressdetail($record);
                    $this->upload_lampiran_lainnya($last_id);
                    $valid = true;
                    $msg = "Data success Send";
                } else {
                    $valid = false;
