 ...
 <section class="content">
      <div class="row">
        ...
        <form id="formdata" class="form-horizontal" data-parsley-validate="" novalidate="" autocomplete="off">
            ...
            <!-- <input type="text" name="end_date" id="end_date" value="<?php echo $get_enddate ?>" /> -->
                           ...
                           $sql ="SELECT * FROM lessential_accessapps.useraccess WHERE 	KodeSeksi = '$user' GROUP BY namaKaryawan ";
                                        ...
                                        if ($query->num_rows() > 0) {
                                          ...
                                          foreach ($query->result() as $row) {?>
                                            <option <?php if($row->KodeSeksi == $this->session->userdata('ses_KodeSeksi')){ echo 'selected="selected"'; } ?> value="<?php echo $row->namaKaryawan ?>"><?php echo $row->namaKaryawan?>[$i] </option>
                                        <?php }
                                        }
                                      }
                            ?>
...
<script type="text/javascript">
    ...
    var enddate= '<?php echo $get_enddate; ?>';
    var dateToday = new Date(enddate);
